...
当前可用实例运行在 NAS，默认访问和健康检查使用：

```bash
http://192.168.31.213:10309
curl -fsS http://192.168.31.213:10309/api/config
```

除非用户明确要求本地调试，不要启动本机 `media-refiner` 容器，也不要把 `127.0.0.1:10309` 当成目标服务。
